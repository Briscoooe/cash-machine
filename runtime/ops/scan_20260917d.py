# 2026-09-17 scan: pull 150 volume-sorted events, filter, list candidates
...
for ev in events:
    ...
    for m in ev.get("markets", []):
        ...
        cands.append({
            ...
            "condition_id": m.get("conditionId"), "desc": (m.get("description") or ev.get("description") or "")[:400],
        })

cands.sort(key=lambda c: -c["vol24"])
json.dump({"n_events": len(events), "cands": cands}, open("/root/cash-machine/runtime/ops/scan_20260917d_cands.json", "w"), indent=1)
print("events:", len(events), "candidates:", len(cands))
for c in cands[:40]:
    print("%.3f | %s | %s | vol24 %d | end %s" % (c["yes"], c["question"][:90], c["slug"][:60], c["vol24"], c["end"]))
